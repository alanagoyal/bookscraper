...
const getRecommendationsForPersonTool = createTool({
  ...
  execute: async ({ context }) => {
    const { data: person, error: personError } = await supabase.from("people").select("id").eq("full_name", context.full_name).single();
    if (personError) {
    ...
    const { data: rawRecommendations, error: queryError } = await supabase.from("recommendations").select(`
        id,
        book_id,
        person_id,
        source,
...
const getRecommendationsForBookTool = createTool({
  ...
  execute: async ({ context }) => {
    const { data: books, error: bookError } = await supabase.from("books").select("id").ilike("title", context.title).ilike("author", context.author).limit(1);
    if (bookError) {
...
const getRecommendationsForTwoPeopleTool = createTool({
  ...
  execute: async ({ context }) => {
    const { data: person1, error: error1 } = await supabase.from("people").select("id").ilike("full_name", context.person1_name).limit(1);
    if (error1) {
    ...
    const { data: person2, error: error2 } = await supabase.from("people").select("id").ilike("full_name", context.person2_name).limit(1);
    if (error2) {
