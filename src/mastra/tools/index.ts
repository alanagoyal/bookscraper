...
export const getRecommendationsForPersonTool = createTool({
  ...
  execute: async ({ context }) => {
    ...
    if (personError) {
      throw new Error(
        `Error finding person by full name: ${personError.message}`
      );
    }
    ...
    const recommendations =
      (rawRecommendations as RawRecommendation[] | null)
        ?.map((rec) => {
...
export const getRecommendationsForBookTool = createTool({
  ...
  execute: async ({ context }) => {
    ...
    if (!books || books.length === 0) {
      throw new Error(
        `No book found with title "${context.title}" by ${context.author}`
      );
    }
...
export const getTopOverlappingRecommendersTool = createTool({
  ...
  description:
    "Get top pairs of recommenders who recommend the most books in common",
...
export const getGenreOutliersInTypeTool = createTool({
  ...
  description:
    "Find people who recommend only one genre while others in their type recommend many",
  inputSchema: z.object({}),
  outputSchema: z.object({
    outliers: z.array(
      z.object({
...
export const getInfluentialRecommendersTool = createTool({
  ...
  description:
    "Find people whose book recs show up most in other people’s lists (influence score)",
  inputSchema: z.object({ limit: z.number().default(10) }),
  outputSchema: z.object({
    influencers: z.array(
      z.object({
  ...
  execute: async ({ context }) => {
    const { data, error } = await supabase.rpc("get_influential_recommenders", {
      limit_arg: context.limit,
    });
    if (error) throw new Error(error.message);
...
export const getSimilarPeopleByDescriptionEmbeddingTool = createTool({
  ...
  description:
    "Find the top 3 people with the most similar description-based book taste to a given person",
  inputSchema: z.object({
    person_full_name: z.string(),
  }),
  outputSchema: z.object({
    people: z.array(
      z.object({
  ...
  execute: async ({ context }) => {
    const { data: person, error: personError } = await supabase
      .from("people")
      .select("id")
      .eq("full_name", context.person_full_name)
      .single();
...
export const getSimilarBooksToBookByDescriptionTool = createTool({
  ...
  description:
    "Find the top 3 books most similar in description to a given book",
  inputSchema: z.object({
    book_title: z.string(),
  }),
  outputSchema: z.object({
...
export const getTopSimilarBooksWithOverlapTool = createTool({
  ...
  description:
    "Get the top semantically similar books and how many recommenders they share",
  inputSchema: z.object({
    limit: z.number().default(10),
  }),
  outputSchema: z.object({
    pairs: z.array(
      z.object({
        book1_id: z.string(),
        book1_title: z.string(),
        book2_id: z.string(),
        book2_title: z.string(),
        similarity: z.number(),
        shared_recommender_count: z.number(),
      })
    ),
  }),
  execute: async ({ context }) => {
